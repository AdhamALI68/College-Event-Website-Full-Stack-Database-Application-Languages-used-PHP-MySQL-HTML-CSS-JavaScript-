...
// Get user theme from consistent session key
...
$query = "
    ...
    WHERE (
        ...
        OR (e.type = 'rso' AND EXISTS (
            SELECT 1 FROM rso_members rm WHERE rm.rso_id = e.rso_id AND rm.user_id = $user_id
        ))
    )
    AND e.approved = 1
    AND CONCAT(e.event_date, ' ', e.start_time) >= '$currentDateTime'
